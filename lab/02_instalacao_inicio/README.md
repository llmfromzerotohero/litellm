...
4. Modelos do Ollama baixados localmente (ex: `llama3.2:3b`).
...
Com o serviço do Ollama em execução localmente em sua máquina, vamos iniciar o proxy do LiteLLM apontando para o modelo local `llama3.2:3b` (ou outro de sua escolha):

```bash
(venv) litellm --model ollama/llama3.2:3b \
               --api_base http://localhost:11434 \
               --port 4000
```

> [!IMPORTANT]
> Certifique-se de que o Ollama está ativo na porta padrão `11434` e que o modelo especificado já foi previamente baixado através de `ollama run llama3.2:3b`.
